...
impl DnsProcessor {
    ...
    #[inline]
    pub(super) fn format_domain_name(name: &Name) -> DnsName255 {
        let mut formatted = DnsName255::default();

        if Self::write_domain_name(name, &mut formatted) {
            return formatted;
        }

        DnsName255::new(Self::remove_trailing_dot(&name.to_ascii())).unwrap_or_default()
    }

    fn write_domain_name(name: &Name, output: &mut DnsName255) -> bool {
    ...
    fn write_label_ascii(label: &[u8], output: &mut DnsName255) -> bool {
    ...
    fn write_ascii_byte(byte: u8, is_first: bool, output: &mut DnsName255) -> bool {
        match byte {
    ...
    fn write_octal_escape(byte: u8, output: &mut DnsName255) -> bool {
    ...
    fn read_wire_domain_name(
        dns_data: &[u8],
        cursor: &mut usize,
    ) -> Result<DnsName255, &'static str> {
        let mut output = DnsName255::default();
